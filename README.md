...
```bash
sam-ts-package-image$ sam build
```
...
```bash
sam-ts-package-image$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-ts-package-image$ sam local start-api
sam-ts-package-image$ curl http://localhost:3000/
```
...
```bash
sam-ts-package-image$ sam logs -n HelloWorldFunction --stack-name sam-ts-package-image --tail
```
...
```bash
sam-ts-package-image$ cd hello-world
...
```bash
sam delete --stack-name sam-ts-package-image
```
